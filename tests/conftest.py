...
def fake_measure_data(initial_data, base_datapoint_time):
    for (build_id, version) in (('20170629075044', '55.0'),
        ...
        for (timestamp, value, usage_hours, client_count) in (
                (base_datapoint_time - datetime.timedelta(minutes=10), 100, 20, 100),
                (base_datapoint_time - datetime.timedelta(minutes=5), 10, 16, 101),
                (base_datapoint_time, 10, 20, 102)):
